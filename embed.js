...
const VOICE_PROMPT_ENDPOINT = 'https://rucrak-crew-chief.vercel.app/api/voice-prompt'; // change alongside CHAT_ENDPOINT if hosted separately
const CART_CHECKOUT_URL_ENDPOINT = 'https://rucrak-crew-chief.vercel.app/api/cart-checkout-url'; // change alongside CHAT_ENDPOINT if hosted separately
...
// Real bug this fixes: the Continue Shopping button previously just linked
// to the site's homepage, relying on a browser cookie to make that page
// show the right cart -- but that exact cookie-sync approach was directly
// tested earlier in this project and confirmed NOT to work on this store.
// Voice-mode adds also never taught the browser the resulting checkoutUrl
// at all, since that whole exchange happens server-side. Real, confirmed
// symptom: customer said the item genuinely wasn't there when they clicked
// through, even though server logs confirmed the add itself succeeded --
// they were just looking at a cart the cookie-sync never actually pointed
// them to correctly.
//
// Fix: look up the cart's real, current checkout URL directly on click and
// navigate there, instead of hoping a cookie makes a generic homepage link
// show the right thing. The new tab is opened synchronously, immediately
// on click, and only its destination is set once the lookup resolves --
// opening a tab only AFTER an async fetch completes commonly gets blocked
// by browser popup blockers, since by then it's no longer directly tied to
// the user's click gesture.
const continueShoppingLink = root.getElementById('continueShoppingLink');
if(continueShoppingLink){
  continueShoppingLink.addEventListener('click', function(e){
    e.preventDefault();
    const fallbackUrl = 'https://rucrak.com/';
    const newTab = window.open('', '_blank');
    const cartId = getStoredCartId();
    if(!newTab){
      // Popup blocked outright -- fall back to a normal same-tab navigation
    ...
    fetch(`${CART_CHECKOUT_URL_ENDPOINT}?cartId=${encodeURIComponent(cartId)}`)
      .then(res => { if(!res.ok) throw new Error(`Lookup failed (${res.status})`); return res.json(); })
      .then(data => { newTab.location.href = data.checkoutUrl || fallbackUrl; })
      .catch(err => {
        console.error('Could not look up real cart checkout URL, falling back to homepage:', err);
        newTab.location.href = fallbackUrl;
      });
  });
}
...
function startVoiceMode(){
  ...
  vapiReadyPromise
    .then(async (vapi)=>{
      // Real change: the assistant's system prompt is now built fresh
      // server-side on every call (see api/voice-prompt.js), with live
      // Shopify pricing baked in, instead of being frozen at whatever
      // static text was last manually pasted into the Vapi dashboard.
      // Confirmed directly against Vapi's own API schema before building
      // this: assistantOverrides.model and .voice are separate fields --
      // overriding model.messages here does NOT touch the assistant's
      // configured voice/tone, which stays exactly as set in the
      // dashboard. If this fetch fails for any reason, falls back to
      // starting the call with no prompt override at all, which uses
      // whatever's saved in the dashboard -- a real working fallback, not
      // a broken call, just without the live-pricing benefit for that one
      // call.
      ...
      if(!voiceCallActive){
        ...
        const overrides = { variableValues: { priorContext: contextForCall, cartId: cartIdForCall } };
        try {
          const promptRes = await fetch(VOICE_PROMPT_ENDPOINT);
          if(promptRes.ok){
            const { systemPrompt } = await promptRes.json();
            if(systemPrompt){
              // Real bug this fixes: Vapi rejects the whole call with a 400
              // if `model` is overridden at all without ALSO including
              // provider and model -- confirmed directly from the actual
              // error message in production. These must match what's
              // configured for this assistant in the Vapi dashboard
              // (Model tab: Anthropic / Claude Haiku 4.5) -- if that ever
              // changes in the dashboard, this needs to change to match.
              overrides.model = {
                provider: 'anthropic',
          ...
          console.error('Could not fetch live voice prompt, falling back to the dashboard-saved prompt for this call (non-fatal):', err);
        }
        const startResult = vapi.start(CREW_CHIEF_ASSISTANT_ID, overrides);
