...
8. **Front vs. back mounting plate position — CONFIRMED DIRECTLY BY RUCRAK'S CEO (Jason Morgan), treat as authoritative:** which side the mounting plate attaches to (front of the vertical uprights vs. back) is determined ONLY by wheel offset/reach — i.e., whether the plate can physically align with the wheel's lug pattern given how far that specific wheel sits in or out. Standard/stock-offset wheels typically mount front; deep-offset aftermarket wheels typically need the plate moved to the back so it can reach the lugs. **This choice does NOT change the 5-7/8" wheel-depth threshold, and does NOT affect whether stud extensions are needed.** That threshold is fixed and applies the same way regardless of which side the plate mounts to. Do not tell a customer that switching mounting sides can help them avoid needing the Mounting Stud Extensions — that's incorrect. The extensions are needed purely based on the lug-stud-to-outer-tire measurement, full stop.
9. The stud length itself is identical across all standard rucRak kits regardless of vehicle or mounting side — confirmed on both a Bronco (stock wheel, front-mounted) and a Jeep (aftermarket deep-offset wheel, back-mounted) build, neither of which needed extensions. Cover-plate/excess-stud-length interference (mentioned in the older Gen V manual as a rare edge case requiring trimming a stud or using a spacer) has not actually come up as a real issue in practice — treat it as a theoretical rare-case footnote, not a practical concern to raise proactively.
